...
let bookmarks = JSON.parse(localStorage.getItem("n400_bookmarks_v2")) || {
  personal: [],
  part9: [],
  glossary: [],
};
...
async function Data() {
  try {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      const catVal = matches[4]
        ? parseInt(matches[4].replace(/^"|"$/g, "").trim())
        : 0;
...
function goToXiaohongshu() {
  const myRedBookUrl =
    "https://www.xiaohongshu.com/user/profile/631f3bfd00000000230254b1";
...
function speakText(text, showAnim = false) {
  ...
  function playNext() {
    if (currentSegment < segments.length) {
      ...
      const needsRateBoost =
        isChromeLike && selectedVoice && !selectedVoice.lang.startsWith("en");
...
function speakGlossaryPhrase(word) {
  ...
  const isChromeLike =
    ...
    !navigator.userAgent.includes("Safari");
  const needsRateBoost =
    isChromeLike && bestVoice && !bestVoice.lang.startsWith("en");
  ...
  u1.onend = () =>
    audioSequenceTimeouts.push(setTimeout(() => synth.speak(u2), 200));
  u2.onend = () =>
    audioSequenceTimeouts.push(setTimeout(() => synth.speak(u3), 200));
...
function switchTab(tab) {
  currentTab = tab;
  document
    .getElementById("tab-personal")
    .classList.toggle("active", tab === "personal");
  document
    .getElementById("tab-part9")
    .classList.toggle("active", tab === "part9");
  document
    .getElementById("tab-glossary")
    .classList.toggle("active", tab === "glossary");
  renderBookmarkList();
}
...
function renderBookmarkList() {
  ...
  list.forEach((val) => {
    ...
    if (currentTab === "glossary") {
      ...
      if (found)
        displayText = `<b>${found.word}</b><br><span style="font-size:14px;color:#666">${found.chinese}</span>`;
    }
